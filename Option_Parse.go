...
func (o *Option) process(short rune, long, arg string) (success bool, err error) {
	...
	for _, alt := range o.allAlternates() {
		...
		if !alt.checkShort(short) && !alt.checkLong(long) {
			continue
		}

		if success == true {
			err = errors.New("Only one of the alternates of '" + alt.group + "' can be specified")
			return
		}
...
func (o *Option) validate(get GetOption) (err error) {
	for _, alt := range o.allAlternates() {
		for _, validator := range alt.validators {
			err = validator(alt, get)
			if err != nil {
				return
			}
		}
